...
with st.expander("⚙️ 分析設定（どの会社を調べる？）", expanded=True):
    ...
    知っている会社から選ぶか、会社のコード（記号）を直接入力できます。<br>
    例：トヨタ = 7203.T、Apple = AAPL
    </div>
    """, unsafe_allow_html=True)
    ...
    if input_type == "人気の会社から選ぶ":
        ...
        st.markdown("""
        <div class="tip-box">
        💡 <strong>ヒント：</strong> 日本の会社は最後に「.T」が付きます（例：7203.T）
        </div>
        """, unsafe_allow_html=True)
...
with st.expander("🔧 詳細設定（上級者向け）"):
    ...
    株価のグラフを見て「上がりそう」「下がりそう」を判断するための道具です。<br>
    数学を使って、人間には見えないパターンを見つけてくれます！
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown("""
        <div class="tip-box">
        📊 <strong>これは何？</strong> 最近の株価の平均です<br>
        <strong>⬆️ 高くすると：</strong> ゆっくり動く線になる<br>
        <strong>⬇️ 低くすると：</strong> 素早く動く線になる<br>
        <strong>👍 おすすめ：</strong> 初心者は20のままでOK
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="tip-box">
        📊 <strong>これは何？</strong> 株が「買われすぎ」か「売られすぎ」かを0-100で表示<br>
        <strong>⬆️ 高くすると：</strong> ゆっくり反応する（安定）<br>
        <strong>⬇️ 低くすると：</strong> 素早く反応する（敏感）<br>
        <strong>👍 おすすめ：</strong> 14のままでOK
        </div>
        """, unsafe_allow_html=True)
