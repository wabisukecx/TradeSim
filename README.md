...
- JDK 17 以上
- Android Studio または VS Code（Flutter 拡張機能）
...
### fat APK（全アーキテクチャ対応・単一ファイル）

```bash
flutter build apk --release
```

デフォルトで fat APK が生成されます。ファイルサイズは大きくなりますが、どのデバイスでも動作します。
...
```
build/app/outputs/flutter-apk/app-armeabi-v7a-release.apk   # 32bit ARM
build/app/outputs/flutter-apk/app-arm64-v8a-release.apk     # 64bit ARM（現行主流）
build/app/outputs/flutter-apk/app-x86_64-release.apk        # x86_64（エミュレーター等）
```

>  一般的な Android スマートフォンには `arm64-v8a` を使用してください。
