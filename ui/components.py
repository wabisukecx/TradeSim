...
class UIComponents:
    ...
    def render_technical_settings() -> Dict[str, int]:
        ...
        UIComponents.render_explanation_box(
            "🔬 テクニカル指標って何？",
            "株価のグラフを見て「動きのパターン」を数値化する道具です。<br>" +
            "数学を使って、人間には見えにくいパターンを可視化してくれます！"
        )
        ...
        with col1:
            ...
            UIComponents.render_tip_box(
                "📊 短期移動平均とは",
                "最近の株価の平均です<br>" +
                "⬆️ 高くすると：ゆっくり動く線になる<br>" +
                "⬇️ 低くすると：素早く動く線になる<br>" +
                "👍 おすすめ：初心者は20のままでOK"
            )
        ...
        with col2:
            ...
            UIComponents.render_tip_box(
                "📊 ボリンジャーバンドとは",
                "株価の「普通の範囲」を表示する線<br>" +
                "⬆️ 高くすると：広い範囲を「普通」と判断<br>" +
                "⬇️ 低くすると：狭い範囲を「普通」と判断<br>" +
                "👍 おすすめ：20のままでOK"
            )
    ...
    def render_backtest_settings() -> Dict[str, float]:
        ...
        UIComponents.render_explanation_box(
            "🎮 学習シミュレーションって何？",
            "「もしこのルールで取引していたら、結果はどうなっていた？」を計算してくれます。<br>" +
            "実際のお金は使わないので安心です！教育目的のシミュレーションです。"
        )
        ...
        with col1:
            ...
            UIComponents.render_tip_box(
                "💰 仮想初期資金",
                "シミュレーション開始時の仮想資金<br>" +
                "⬆️ 多くすると：大きく変動する結果になる<br>" +
                "⬇️ 少なくすると：小さく変動する結果になる"
            )
            ...
            UIComponents.render_tip_box(
                "⚡ リスク許容率",
                "1回の取引でどのくらいリスクを取るか<br>" +
                "⬆️ 高くすると：積極的な取引（ハイリスク）<br>" +
                "⬇️ 低くすると：慎重な取引（ローリスク）<br>" +
                "👍 おすすめ：初心者は2%以下"
            )
        ...
        with col2:
            ...
            UIComponents.render_tip_box(
                "🛡️ 損切り率",
                "「これ以上下がったら売る」というルール<br>" +
                "⬆️ 高くすると：我慢強く保有し続ける<br>" +
                "⬇️ 低くすると：早めに損切りする<br>" +
                "👍 おすすめ：5-10%が一般的"
            )
            ...
            UIComponents.render_tip_box(
                "🎯 利益確定率",
                "「これだけ上がったら売る」というルール<br>" +
                "⬆️ 高くすると：長期保有（もっと上がるまで待つ）<br>" +
                "⬇️ 低くすると：早めに利益確定（少し上がったら売る）<br>" +
                "👍 おすすめ：損切り率の2-3倍"
            )
        ...
        UIComponents.render_tip_box(
            "💳 取引手数料率",
            "株を買ったり売ったりする時の手数料<br>" +
            "⬆️ 高くすると：現実的だけど、利益が減る<br>" +
            "⬇️ 低くすると：理想的だけど、現実とは違う<br>" +
            "👍 おすすめ：0.1%（大手ネット証券の平均）"
        )
